...
CREATE TABLE registrations (
    ...
    CONSTRAINT fk_registrations_user
        ...
        ON UPDATE CASCADE
        ON DELETE SET NULL,
    UNIQUE KEY uk_registrations_event_email (event_id, email),
    UNIQUE KEY uk_registrations_token (token),
    KEY idx_registrations_event_registered (event_id, registered_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Index utile pour searchEvents(), qui filtre les evenements par plage de date
-- et categorie puis trie par event_date. MySQL peut ainsi reduire rapidement
-- le nombre de lignes candidates avant le tri et la jointure registrations.
...
INSERT INTO registrations (event_id, user_id, name, email, token, registered_at) VALUES
    (
        1,
        2,
        'Yassine El Fassi',
